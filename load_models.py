...
def load_quantized_model_qptq(model_id, model_basename, device_type):
    ...
    if sys.platform == "darwin":
        print("GPTQ models will NOT work on Mac devices. Please choose a different model.")
        return None, None
...
def load_quantized_model_awq(model_id):
    """
    ...
    if sys.platform == "darwin":
        print("AWQ models will NOT work on Mac devices. Please choose a different model.")
        return None, None

    # The code supports all huggingface models that ends with AWQ.
    print("Using AutoModelForCausalLM for AWQ quantized models")
